...
1. Deploy the Tanzu Operations Manager v3.0.40
1. Deploy the Bosh Director
1. Deploy the Tanzu Kubernetes Grid Integrated Edition v1.21.0
1. Deploy a TKGI cluster
1. Deploy a simple NGINX webserver workload and expose the service via a LoadBalancer
...
### Deploy the Tanzu Operations Manager v3.0.40
...
1. In the "Select an OVF Template" screen, select "Local file" and then click on "Upload Files" to upload the OVA file that was downloaded.  Click "Next".
...
1. Set up "Internal Authentication".  Provide a password for the "admin" account.  Provide a passphrase as well.  Click "OK" to complete the authentication setup.
1. At the Ops Manager Login, enter the "admin" user and password.

### Deploy the Bosh Director

1. In the Ops Manager UI, click the Director tile to see the settings page.
...
### Deploy the Tanzu Kubernetes Grid Integrated Edition v1.21.0
...
1. In Plan 1 tab, check the "az1" checkbox under Master/ETCD Availability Zones and also under Worker Availability Zones.  Click "Save".
...
1. Follow the instructions on [how to generate and register a NSX Principal Identity](https://techdocs.broadcom.com/us/en/vmware-tanzu/standalone-components/tanzu-kubernetes-grid-integrated-edition/1-21/tkgi/nsxt-generate-pi-cert.html#certificate-super-user-script){:target="_blank"}
1. Enter the generated Principal Identity cert and key into the NSX Manager Super User Principal Identity Certificate.
1. Enter the NSX Manager CA cert.
1. Enable NAT mode
...
1. Enter 192.168.86.34 and the Nodes DNS.
